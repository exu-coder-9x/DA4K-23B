...
# UI theme matched to D4K3 logo — deep purple / violet neon reaper aesthetic
st.markdown(
    """
    <style>
    ...
    .stApp {
        background: radial-gradient(ellipse at 30% 0%, #1a0530 0%, #0a0214 45%, #000000 100%);
        color: #e9d5ff;
    }

    /* Sidebar */
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #12021f 0%, #07010f 100%);
    ...
    /* Hero */
    .d4k3-hero {
        background: linear-gradient(135deg, rgba(40,8,70,0.85) 0%, rgba(10,2,20,0.95) 100%);
        border: 1px solid #a855f7;
        border-radius: 20px;
        padding: 1.2rem 1.6rem;
        margin-bottom: 1.4rem;
        box-shadow: 0 0 50px rgba(168, 85, 247, 0.25), inset 0 1px 0 rgba(232, 121, 249, 0.2);
        display: flex;
        align-items: center;
        gap: 1.4rem;
    }
    ...
    .d4k3-title {
        ...
        text-shadow: 0 0 22px rgba(232, 121, 249, 0.65);
    }
    .d4k3-sub {
        margin: 0.3rem 0 0 0;
        color: #a78bfa;
    ...
    .stButton > button:hover, .stFormSubmitButton > button:hover {
        box-shadow: 0 0 24px rgba(217, 70, 239, 0.5) !important;
        transform: translateY(-1px);
    }

    .stTextInput input, .stSelectbox [data-baseweb="select"],
    .stTextArea textarea {
        background: #0f0218 !important;
    ...
    .aStyle a { color: #d946ef !important; text-decoration: none; }
    .aStyle a:hover { text-shadow: 0 0 10px #d946ef; }
...
st.sidebar.markdown(
    ...
    <div style="text-align:center; padding: 0.3rem 0 0.8rem 0;">
      <div style="font-family:Orbitron,sans-serif; font-size:1.35rem; font-weight:700; color:#e879f9; letter-spacing:0.16em; text-shadow:0 0 16px #d946ef;">D4K3 24B</div>
      <div style="font-size:0.72rem; color:#a78bfa; letter-spacing:0.14em; margin-top:0.15rem;">DARK OSINT ENGINE</div>
    </div>
    """,
    unsafe_allow_html=True,
)
st.sidebar.caption("Android · Termux · Orbot ready")
st.sidebar.subheader("⚙ Controls")

def _env_is_set(value) -> bool:
    return bool(value and str(value).strip() and "your_" not in str(value))

# ---------- In-app API configuration (session overrides .env for this run) ----------
_api_defaults = {
    "ui_openai_key": OPENAI_API_KEY or "",
    "ui_anthropic_key": ANTHROPIC_API_KEY or "",
    "ui_google_key": GOOGLE_API_KEY or "",
    "ui_openrouter_key": OPENROUTER_API_KEY or "",
    "ui_ollama_url": OLLAMA_BASE_URL or "",
    "ui_llamacpp_url": LLAMA_CPP_BASE_URL or "",
    "custom_api_url": _app_cfg.CUSTOM_API_BASE_URL or "",
    "custom_api_key": _app_cfg.CUSTOM_API_KEY or "",
    "custom_api_model": _app_cfg.CUSTOM_API_MODEL or "",
}
for _k, _v in _api_defaults.items():
    if _k not in st.session_state:
        st.session_state[_k] = _v or ""

# Apply session values into the live config module so LLM layer sees them
import config as _cfg_mod
_cfg_mod.OPENAI_API_KEY = (st.session_state.get("ui_openai_key") or "").strip() or None
_cfg_mod.ANTHROPIC_API_KEY = (st.session_state.get("ui_anthropic_key") or "").strip() or None
_cfg_mod.GOOGLE_API_KEY = (st.session_state.get("ui_google_key") or "").strip() or None
_cfg_mod.OPENROUTER_API_KEY = (st.session_state.get("ui_openrouter_key") or "").strip() or None
_cfg_mod.OLLAMA_BASE_URL = (st.session_state.get("ui_ollama_url") or "").strip() or None
_cfg_mod.LLAMA_CPP_BASE_URL = (st.session_state.get("ui_llamacpp_url") or "").strip() or None
_cfg_mod.CUSTOM_API_BASE_URL = (st.session_state.get("custom_api_url") or "").strip() or None
_cfg_mod.CUSTOM_API_KEY = (st.session_state.get("custom_api_key") or "").strip() or None
_cfg_mod.CUSTOM_API_MODEL = (st.session_state.get("custom_api_model") or "").strip() or None

# Keep imported names in this module in sync for provider status display
OPENAI_API_KEY = _cfg_mod.OPENAI_API_KEY
ANTHROPIC_API_KEY = _cfg_mod.ANTHROPIC_API_KEY
GOOGLE_API_KEY = _cfg_mod.GOOGLE_API_KEY
OPENROUTER_API_KEY = _cfg_mod.OPENROUTER_API_KEY
OLLAMA_BASE_URL = _cfg_mod.OLLAMA_BASE_URL
LLAMA_CPP_BASE_URL = _cfg_mod.LLAMA_CPP_BASE_URL

with st.sidebar.expander("🔑 API Configuration", expanded=False):
    st.caption("Keys entered here apply for this session (override .env). Nothing is written to disk.")
    st.text_input("OpenAI API Key", key="ui_openai_key", type="password", placeholder="sk-...")
    st.text_input("Anthropic API Key", key="ui_anthropic_key", type="password", placeholder="sk-ant-...")
    st.text_input("Google / Gemini API Key", key="ui_google_key", type="password")
    st.text_input("OpenRouter API Key", key="ui_openrouter_key", type="password")
    st.text_input("Ollama Base URL", key="ui_ollama_url", placeholder="http://127.0.0.1:11434")
    st.text_input("llama.cpp Base URL", key="ui_llamacpp_url", placeholder="http://127.0.0.1:8080")
    st.markdown("---")
    ...
    st.text_input(
        ...
        help="Groq, Mistral, LM Studio, Together, etc.",
    )
    ...
    st.text_input(
        ...
        help="Required if the provider does not list models via /v1/models.",
    )
...
if not model_options:
    st.sidebar.error(
        ...
        "Open **🔑 API Configuration** above and add at least one key or local endpoint, "
        "or set keys in your `.env` file."
    )
    st.stop()
...
_providers = [
    ...
    ("Custom API",  _cfg_mod.CUSTOM_API_BASE_URL, False),
]
...
_logo_path = Path(".github/assets/d4k3_logo.png")
_logo_b64 = ""
...
st.markdown(
    f"""
    <div class="d4k3-hero">
      <img src="data:image/png;base64,{_logo_b64}" alt="D4K3 24B" />
      <div>
        <p class="d4k3-title">D4K3 24B</p>
        <p class="d4k3-sub">AI-driven dark-web OSINT · query → search → scrape → brief</p>
        <span class="d4k3-badge">TERMUX + ORBOT</span>
        <span class="d4k3-badge">MULTI-LLM</span>
        <span class="d4k3-badge">TOR SOCKS</span>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with st.form("search_form", clear_on_submit=True):
    ...
    query = col_input.text_input(
        "Query",
        placeholder="Describe the investigation target or keyword…",
...
def _render_investigation_body(inv):
    ...
    st.subheader("🔎 Findings", anchor=None, divider="gray")
...
def _render_chat_panel(inv):
    ...
    st.divider()
    st.subheader("💬 Follow-up Chat", anchor=None, divider="gray")
