...
def check_tor_proxy():
    """Test that the Tor SOCKS5 proxy is accepting connections (supports Orbot/Termux)."""
    host = app_config.TOR_SOCKS_HOST or "127.0.0.1"
    try:
        port = int(app_config.TOR_SOCKS_PORT or 9050)
    ...
    try:
        ...
        return {"status": "up", "latency_ms": latency_ms, "error": None, "endpoint": f"{host}:{port}"}
    except Exception as e:
        return {"status": "down", "latency_ms": None, "error": str(e), "endpoint": f"{host}:{port}"}
